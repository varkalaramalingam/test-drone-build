...
function AddEditAwards({ isAdd, initialData, getAwardsData, titleId }) {
  ...
  const onSubmit = (fields) => {
    io({
      method: "post",
      url: `/api/mastermeta/${titleId}/awards/${!isAdd ? initialData.id : ""}`,
      data: fields,
    })
      .then(({ response }) => {
        message.success(
  ...
  const getAwards = () => {
    ...
    const getCategory = () => {
      io({ method: "get", url: `/api/category/{category_type}?category_type=1` })
        .then(({data}) => {
        ...
        >
          {({ handleSubmit, handleChange, values, errors, touched, setFieldValue}) => (
            ...
            <Form.Item label="Category" required>
             ...
             <Row span={24} gutter={10} style={{marginTop:"15px"}}>
              <Col span={12}>
